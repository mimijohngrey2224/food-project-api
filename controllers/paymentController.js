...
exports.initiatePayment = async (req, res) => {
  ...
  try {
    ...
    if (data.status === "success") {
      ...
      console.log('Sending error response to frontend:', { error: data.message || "Payment Initiation Failed" });
...
exports.verifyPayment = async (req, res) => {
  ...
  try {
    ...
    const data = await response.json();
    if (data.status === "success") {
      const cart = await orderModel.findOne({ user: req.user.id }).populate("products.product");
      ...
      await order.save();
      await orderModel.findOneAndDelete({ user: req.user.id });
